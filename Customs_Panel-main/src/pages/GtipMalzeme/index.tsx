...
export default function GtipMalzemePage() {
  ...
  const [selectedCustomerId, setSelectedCustomerId] = useState('arcelik');
  ...
  // load customers once
  useEffect(() => {
    gtipService.getCustomers().then((data) => setCustomers(data));
  }, []);
  ...
  function handleNewRecord(record: Omit<MaterialRecord, 'id' | 'customerId'>) {
    const newRec: MaterialRecord = {
      ...record,
      id: String(Date.now()),
  ...
  function handleImport(count: number) {
    toast(`Dosya yüklendi · ${count} malzeme okundu, Onay Bekleyen olarak eklendi`);
    ...
    const imported: MaterialRecord[] = Array.from({ length: count }, (_, i) => ({
      id: `imp-${Date.now()}-${i}`,
      materialNo: `MLZ-IMP-${Math.floor(100000 + Math.random() * 900000)}`,
      description: '(İçe aktarılan malzeme)',
      gtipNo: '0000.00.00.00.00',
