...
async function main() {
  ...
  try {
    ...
    // A logical document must not borrow a completed run from another physical file.
    const foreignPhysicalId = new mongoose.Types.ObjectId();
    const logical = await LogicalDocumentModel.findOne({declarationId});
    assert(logical);
    logical.uploadedFileId = foreignPhysicalId;
    await logical.save();
    const mismatched = await tryOrchestrateDeclarationAfterProcessing({companyId, declarationId});
    assert.deepEqual(mismatched, {status: "NOT_READY", reason: "RUN_DOCUMENT_OWNERSHIP_MISMATCH"});
    assert.equal(await DeclarationFieldResolutionRunModel.countDocuments({declarationId}), 0);
    logical.uploadedFileId = uploadedFileId;
    await logical.save();
    ...
    assert.equal(await DeclarationFieldResolutionRunModel.countDocuments({declarationId}), 1);
    console.log(JSON.stringify({event: "foundation-6.12.run-document-ownership.passed",
      ...
      guardrails: {runDocumentOwnershipEnforced: true, mismatchCreatesAuditRecord: false, staleRunExcluded: true, malformedExtractionRejectedBeforeWrite: true,
