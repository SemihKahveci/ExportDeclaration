...
export async function authContextMiddleware(req: Request, _res: Response, next: NextFunction): Promise<void> {
  ...
  if (!payload || !mongoose.isValidObjectId(payload.sub) || !mongoose.isValidObjectId(payload.companyId)) {
    next(new HttpError(401, "Oturum bulunamadı veya süresi dolmuş."));
    return;
  }
  ...
  if (!user) {
    next(new HttpError(401, "Kullanıcı aktif değil veya bulunamadı."));
    return;
  }
