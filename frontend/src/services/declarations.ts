...
export const declarationsService = {
  ...
  get: async (id: string): Promise<Declaration | null> => {
    const live = await import('./liveApi').then((m) => m.fetchPanelDeclarations());
    if (live?.length) return live.find((d) => d.id === id) ?? null;
    await delay(80);
    return MOCK.find((d) => d.id === id) ?? null;
  },
  create: async (): Promise<Declaration> => {
    if ((await import('./liveApi')).liveApiEnabled()) {
      try {
        return await (await import('./liveApi')).apiCreateDeclaration();
      } catch {
        /* mock fallback */
      }
    }
    await delay(100);
    const id = `dcl-${Date.now()}`;
    const row: Declaration = { id, ref: `DCL-${id.slice(-6)}`, customer: '—', status: 'taslak', createdAt: new Date().toISOString() };
    MOCK.push(row);
    return row;
  },
};
...
export const beyannameListeService = {
  ...
  uploadDocument: async (payload: DocumentUploadPayload): Promise<void> => {
    if (payload.file && (await import('./liveApi')).liveApiEnabled()) {
      try {
